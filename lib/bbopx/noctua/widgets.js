...
bbopx.noctua.widgets.edit_annotations_modal = function(annotation_config,
	...
	// First, select function. All evidence is handled the same
	// way (besides target);
	var delegate_function = null;
	if( is_ev_p ){
	    // Evidence is only defined for individuals and facts.
	    if( entity_type == 'individual' &&  entity_op == 'add' ){
		manager.add_individual_evidence(model_id, args['id'],
					       );
	    }else if( entity_type == 'individual' &&  entity_op == 'remove' ){
	    }else if( entity_type == 'fact' &&  entity_op == 'add' ){
	    }else if( entity_type == 'fact' &&  entity_op == 'remove' ){
	    }else{
		throw new Error('no idea what annotation op was intended');
	    }	    
	}else{
